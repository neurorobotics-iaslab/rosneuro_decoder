...
class TestGenericDecoder: public GenericDecoder {
    public:
        ...
        std::string path(void) { return ""; }
        std::vector<int> classes(void) { return std::vector<int>({}); }
};
...
TEST_F(TestGenericDecoderSuite, LoadVectorOfVector){
    ...
    ASSERT_TRUE(genericDecoder->load_vectorOfVector("1 2 3;4 5 6;", out));
...
TEST_F(TestGenericDecoderSuite, LoadEigen){
    ...
    ASSERT_TRUE(genericDecoder->load_eigen("1 2 3;4 5 6;7 8 9;", out));
...
TEST_F(TestGenericDecoderSuite, LoadEigenFail){
    Eigen::MatrixXf out;
    ASSERT_FALSE(genericDecoder->load_eigen("1 2 3;4 5 6;7 8 9", out));
    out = Eigen::MatrixXf::Zero(3, 3);
    ASSERT_FALSE(genericDecoder->load_eigen("1 2 3;4 5 6;7 8;", out));
}

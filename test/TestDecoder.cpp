    ...
    namespace decoder {
        class TestDecoder : public Decoder {
            public:
                ...
                MOCK_METHOD0(getClasses, std::vector<int>());
        ...
        TEST_F(TestDecoderSuite, Path){
            EXPECT_CALL(*decoder, getPath()).Times(1).WillOnce(testing::Return("test"));
            ASSERT_EQ(decoder->getPath(), "test");
        }

        TEST_F(TestDecoderSuite, Classes){
            EXPECT_CALL(*decoder, getClasses()).Times(1).WillOnce(testing::Return(std::vector<int>({})));
            ASSERT_EQ(decoder->getClasses(), std::vector<int>({}));
        }
        ...
        TEST_F(TestDecoderSuite, VectorToEigen){
            ...
            Eigen::VectorXf out = decoder->vectorToEigen(in);
        ...
        TEST_F(TestDecoderSuite, EigenToVector){
            Eigen::VectorXf in(3);
            in << 1, 2, 3;
            std::vector<float> out = decoder->eigenToVector(in);
            ASSERT_EQ(in.size(), out.size());
            for (int i = 0; i < in.size(); i++)
                ASSERT_EQ(in[i], out[i]);
        }
